...
My code is mainly about a simulated galaxy, and the creative ideas are mainly inspired by our lessons 7 and 8. My idea is very simple. Set a point as the center and then set a plane. There are many points on this plane in different coordinate axes, rotate around the center point with circular trajectory at different speeds, and then set each point with a material and a big frame for the background. For the convenience of viewing, I also added that holding down the mouse can change the angle freely, which can be solved by setting the camera.
Ten maps, one obj, were used in my file!
